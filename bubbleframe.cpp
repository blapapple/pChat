...
BubbleFrame::BubbleFrame(ChatRole role, QWidget *parent)
    ...
    if (m_role == ChatRole::Self) {
        m_pHLayout->setContentsMargins(m_margin, m_margin,
                                       WIDTH_ANGLE + m_margin, m_margin);
    } else {
        m_pHLayout->setContentsMargins(m_margin + WIDTH_ANGLE, m_margin,
                                       m_margin, m_margin);
    }
...
void BubbleFrame::paintEvent(QPaintEvent *e) {
    ...
    if (m_role == ChatRole::Other) {
        ...
        QRect bk_rect =
            QRect(WIDTH_ANGLE, 0, this->width() - WIDTH_ANGLE, this->height());
        painter.drawRoundedRect(bk_rect, 5, 5);
        // 画小三角
        QPointF points[3] = {
            QPointF(bk_rect.x(), 12),
            QPointF(bk_rect.x(), 10 + WIDTH_ANGLE + 2),
            QPointF(bk_rect.x() - WIDTH_ANGLE,
                    10 + WIDTH_ANGLE - WIDTH_ANGLE / 2),
        };
        ...
        // 画气泡
        QRect bk_rect =
            QRect(0, 0, this->width() - WIDTH_ANGLE, this->height());
        painter.drawRoundedRect(bk_rect, 5, 5);
        // 画三角
        QPointF points[3] = {
            QPointF(bk_rect.x() + bk_rect.width(), 12),
            QPointF(bk_rect.x() + bk_rect.width(), 12 + WIDTH_ANGLE + 2),
            QPointF(bk_rect.x() + bk_rect.width() + WIDTH_ANGLE,
                    10 + WIDTH_ANGLE - WIDTH_ANGLE / 2),
        };
        painter.drawPolygon(points, 3);
    }
